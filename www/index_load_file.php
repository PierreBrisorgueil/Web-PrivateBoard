...
if ($_SESSION['lvl'] == 0 || $_SESSION['lvl'] == 1) {
	switch ($cat) {
		...
		case "Autre":
			...
			break;
		case "Docs":
			$resultats = $connexion->query("SELECT * FROM files WHERE cat = 'Docs' ORDER BY propername ASC");
			$resultats->setFetchMode(PDO::FETCH_OBJ);
			break;
		case "":
...
while ($ligne = $resultats->fetch()) {
	...
	if ($ligne2->moyenne == "") {$moyenne = "<center>--</center>";} else { $moyenne = color_note($ligne2->moyenne);}
	...
	if ($cat != "Series" && $cat != "Musiques" && $cat != "autre" && $cat != "" && $cat != "Films" && $cat != "eBooks") {
		...
		for ($j = 0; $j < sizeof($explode); $j++) {
			if (preg_match("#S0#i", $explode[$j]) || preg_match("#S1#i", $explode[$j])) {
				$saison = substr($explode[$j], 0, 3);
			}
		}

		if ($saison != $save_saison) {
			$Output .= "<tr></tr><tr><th colspan='8'>" . $saison . "</th></tr>";
		}
