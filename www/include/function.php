...
function clean_name($namefile, $pathfile) {
	...
	if ($size_name > $size_path) {$selection = $namefile;} else { $selection = $namepath_file;}
	...
	} elseif ($max == $nb_tiret) {$selection = str_replace('-', ' ', $selection);} elseif ($max == $nb_tiret_) {$selection = str_replace('_', ' ', $selection);} elseif ($max == $nb_point) {$selection = str_replace('.', ' ', $selection);}
	...
	for ($j = 0; $j < sizeof($selection); $j++) {
		...
		if ($detect == 0 && $j < (sizeof($selection) - 1)) {
			...
			if ($verif == 1 && $verif2 == 1) {$name .= $selection[$j] . ".";} else { $name .= $selection[$j] . " ";}
...
function color_note($note) {
	if ($note >= 0) {$note = "<font color='#5d7078'><b>" . $note . "</b></font>";} elseif ($note >= 5 && $note < 10) {$note = "<font color='#3c87c0'><b>" . $note . "</b></font>";} elseif ($note >= 10) {$note = "<font color='#00ffe4'><b>" . $note . "</b></font>";} elseif ($note < 0) {$note = "<font color='#c0392b'><b>" . $note . "</b></font>";}
	...
	if ($detection_vote == 0) {
		$vote = '
				<td class="classement" width="70px" >
						<a onClick="jaime1(' . $id . ', ' . $user . ', 1, this)"  style="cursor:pointer;">
...
function detectHD($namefile) {
	$HD = 0;
	if (preg_match("#720p#i", $namefile)) {$HD = 1;} elseif (preg_match("#1080p#i", $namefile)) {$HD = 2;}
	return $HD;
}
...
function detectV($namefile) {
	$v = 0;
	if (preg_match("#vostfr#i", $namefile)) {$v = 1;} elseif (preg_match("#french#i", $namefile) || preg_match("#truefrench#i", $namefile)) {$v = 2;}
	return $v;
}
...
function detectType($namefile) {
	...
	for ($j = 0; $j < sizeof($analyse_cat); $j++) {
		...
		) {$T = 1;} elseif ($string == "flac") {$T = 2;} elseif ($string == "doc" || $string == "DOC") {$T = 3;} elseif ($string == "dvdrip" || $string == "bdrip" || $string == "brrip") {$T = 4;} elseif ($string == "pdf" || $string == "epub" || $string == "ebook" || $string == "pour les nuls") {$T = 5;}
	}
	if ($T == 0) {
		if (preg_match("#flac#i", $namefile) || preg_match("#cd#i", $namefile) || preg_match("#mp3#i", $namefile)) {$T = 2;}
		if (preg_match("#mac#i", $namefile) || preg_match("#macosx#i", $namefile) || preg_match("#apple#i", $namefile) || preg_match("#dmg#i", $namefile)) {$T = 6;}
		...
		if (preg_match("/\.cbz/", $namefile) || preg_match("/\.cbr/", $namefile)) {$T = 5;}
...
function defineCat($code) {
	switch ($code) {
		...
		case '220':$cat = "cat-hd-1080p";return $cat;
			break;
		case '204':$cat = "cat-hd-1080p";return $cat;
			break;
		case '214':$cat = "cat-hd-1080p";return $cat;
			break;
							...
							if ($info['size'] > 100000) // si le fichier fait plus de 1 mo
							{
								...
								if ($etat == 0) {
									...
									if ($detection_db == 0) {
										...
										$_name = $connexion->quote($file);
										$array_clear[] = $file;
										if (isset($proper_name)) {$_propername = $connexion->quote($proper_name);} else { $_propername = $connexion->quote($file);}
...
function TriSectionv4($cat) {
	$table_film = array("100", "120", "200", "220", "024", "014", "004", "104", "124", "204", "214");
	$table_musique = array("002");
	$table_ebooks = array("005", "025");
	$table_series = array("111", "121", "011", "021", "001", "011", "201");
	$table_emission = array("028");
	$table_docs = array("003", "203", "103", "023", "223", "123");

	$table_autre_tous = array_merge($table_film, $table_musique, $table_ebooks, $table_series, $table_emission, $table_docs);

	if (in_array($cat, $table_film)) {$categorie = "Films";} elseif (in_array($cat, $table_musique)) {$categorie = "Musiques";} elseif (in_array($cat, $table_ebooks)) {$categorie = "eBooks";} elseif (in_array($cat, $table_series)) {$categorie = "Series";} elseif (in_array($cat, $table_emission)) {$categorie = "Emissions";} elseif (in_array($cat, $table_docs)) {$categorie = "Docs";} elseif (!in_array($cat, $table_autre_tous)) {$categorie = "Autre";}
